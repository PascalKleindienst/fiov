            ...
            <div class="w-full lg:p-8">
                <div class="mx-auto flex w-full flex-col justify-center space-y-6 sm:w-[350px]">
                    <a href="{{ route('dashboard') }}" class="z-20 flex flex-col items-center gap-2 font-medium lg:hidden" wire:navigate>
                        ...
                        <span class="sr-only">{{ config('app.name', 'Fiov') }}</span>
                    </a>
                    {{ $slot }}
                </div>
            </div>
            <div class="relative hidden h-full flex-col place-items-center p-10 text-white lg:grid dark:border-e dark:border-zinc-600">
                <div class="custom-background absolute inset-0 bg-zinc-600"></div>

                <x-app-logo class="relative z-20 flex items-center fill-white text-lg font-medium" />
            </div>
        </div>
        @fluxScripts
    </body>
